...
public class BoardController {
    ...
    @Operation(summary = "게시판의 편지 리스트 조회", description = "memberId에 해당하는 게시판의 편지 리스트를 조회합니다.")
    @GetMapping("/{memberId}/letters")
    public ResponseEntity<List<LetterDto>> getLettersByMemberId(@PathVariable Long memberId) {
        List<LetterDto> letterDtos = boardService.getLettersByMemberId(memberId); // memberId를 기반으로 편지 리스트 조회
    ...
    @Operation(summary = "편지 작성", description = "memberId에 해당하는 게시판에 편지를 게시합니다.")
    @PostMapping("/{memberId}/letter")
    public ResponseEntity<Letter> createLetter(@PathVariable Long memberId, @RequestBody LetterDto letterDto) {
        // memberId를 사용하여 편지 생성
        Letter createdLetter = boardService.createLetterByMemberId(memberId, letterDto);
